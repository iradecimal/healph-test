...
const DailyIntakeTable = ({ data,type }) => {
      ...
      <div>
        ...
        <div style={{maxWidth:"100%",overflowX:"auto"}}>
          <Table striped bordered hover style={{borderColor:"#9FC856"}}>
            <thead>
              <tr>
                <th onClick={() => handleSort("Mealid")}>Meal ID</th>
                <th onClick={() => handleSort("Date")}>Date</th>
                <th onClick={() => handleSort("Fats")}>Fats</th>
                <th onClick={() => handleSort("Carbohydrates")}>Carbohydrates</th>
                <th onClick={() => handleSort("Protein")}>Protein</th>
                <th onClick={() => handleSort("Foodwaste")}>Food waste</th>
            ...
            <tbody style={{backgroundColor: "black"}}>
              {currentData.map((item, index) => (
                <tr key={index}>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Meal ID: ${item.Mealid}`)}>
                      <div>{item.Mealid}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                  ...
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Fats: ${item.Fats}`)}>
                      <div>{item.Fats}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Carbohydrates: ${item.Carbohydrates}`)}>
                      <div>{item.Carbohydrates}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Protein: ${item.Protein}`)}>
                      <div>{item.Protein}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Food Waste: ${item.Foodwaste}`)}>
                      <div>{item.Foodwaste}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Food Combination: ${item.Foodcombination.join(",")}`)}>
                      <div>{item.Foodcombination.join(",")}</div>
                    </OverlayTrigger>
                  </td>
                  <td>
                    <OverlayTrigger placement="top" overlay={renderTooltip(`Description: ${item.Description}`)}>
