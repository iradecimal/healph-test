  ...
  const weeklystats = await Daily_Intake.aggregate([
    {
      $match: {
        date: {
          $lte: startdate.toISOString().slice(0, 10),
          $gte: lastdate.toISOString().slice(0, 10),
        },
      },
    },
    {
...
exports.dailyRankings = asyncHandler(async (req, res, next) => {
  ...
  const rankings = await Daily_Intake.aggregate([
    //only accept intakes that were submitted
    { $match: { date: dailydate.toISOString().slice(0, 10) } },
    { $sort: { hale: -1 } },
    {
      $lookup: {
...
exports.monthlyRankings = asyncHandler(async (req, res, next) => {
  ...
  let month = month_date.push(dailydate.toISOString().slice(0, 7));
